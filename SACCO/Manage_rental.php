...
<body>
  ...
  <div class="main p-3" id="main">
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        $userDetails = getUserDetails($row['rental_user_id'], $conn);
?>
    <div class="card mb-3 mt-2" id="Print-section">
            <div class="card-body">
                <h5 class="card-title">Rental Request ID: <?php echo $row['rental_id']; ?></h5>
                <p class="card-text">User ID: <?php echo $row['rental_user_id']; ?></p>
                ...
                <p class="card-text">Rental Start Date: <?php echo $row['rental_start_date']; ?></p>
                <p class="card-text">Rental End Date: <?php echo $row['rental_end_date']; ?></p>
                <p class="card-text">Rental Location: <?php echo $row['rental_location']; ?></p>
                <p class="card-text">Number of Matatus: <?php echo $row['rental_num_matatus']; ?></p>
               
                <form action='verify_rental_status.php' method='post'>
                    <input type='hidden' name='rental_id' value='<?php echo $row['rental_id']; ?>'>
                    <label for='rental_approval_status'>Approval Status:</label>
                    <select name='rental_approval_status' id='rental_approval_status' required>
                        <option value='verified'>Approve</option>
                        <option value='rejected'>Reject</option>
                    </select>
                    <label for='rental_price'>Price (in kshs):</label>
                    <input type='number' name='rental_price' id='rental_price' placeholder='Enter Price' required>
